...
class FileProcessor:
    ...
    def _create_pivot_summary_sheet(
        ...
        try:
            ...
            # Write to Excel with index=False (now safe since columns are flat)
...
class CognosAccessReviewApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.filter_var.trace_add("write", self.filter_agencies)
        filter_entry = ctk.CTkEntry(
            setup_scroll, textvariable=self.filter_var, placeholder_text="🔍 Search agencies...", height=40,
            fg_color=get_color("input_bg"), border_color=get_color("input_border"),
            text_color=get_color("text"), corner_radius=8,
            font=ctk.CTkFont(size=13)
        )
        filter_entry.pack(fill="x", padx=5, pady=(0, 12))

        # Control buttons
        button_frame = ctk.CTkFrame(setup_scroll, fg_color="transparent")
        button_frame.pack(fill="x", pady=(0, 10))
        button_frame.grid_columnconfigure((0, 1, 2), weight=1)
        
        ctk.CTkButton(
            button_frame, text="✓ All", command=self.select_all_agencies, height=38, corner_radius=8,
            fg_color=get_color("primary"), text_color=get_color("text_light"),
            hover_color=get_color("primary_hover"), font=ctk.CTkFont(size=13, weight="bold")
        ).grid(row=0, column=0, padx=2, sticky="ew")
        
        ctk.CTkButton(
            button_frame, text="✗ None", command=self.deselect_all_agencies, height=38, corner_radius=8,
            fg_color=get_color("secondary"), text_color=get_color("text_light"),
            hover_color=get_color("secondary_hover"), font=ctk.CTkFont(size=13, weight="bold")
        ).grid(row=0, column=1, padx=2, sticky="ew")
        
        ctk.CTkButton(
            button_frame, text="➕ Add", command=self.add_agency_folder, height=38, corner_radius=8,
            fg_color=get_color("primary"), text_color=get_color("text_light"),
            hover_color=get_color("primary_hover"), font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=2, padx=2, sticky="ew")

        # Scrollable agency list
        self.agency_scrollable_frame = ctk.CTkScrollableFrame(setup_scroll, fg_color=get_color("surface"), corner_radius=8, label_text="Agencies")
        self.agency_scrollable_frame.pack(fill="both", expand=True, padx=5, pady=(0, 10))
        
        # Dictionary to hold agency checkbox references
...
8. Use Reporting for analytics and exports"""
    ...
    def show_about(self):
        ...
        about_dialog.title("About Cognos Review")
        ...
        # Title
        title = ctk.CTkLabel(
            about_dialog, text="Cognos Access Review Tool",
            font=ctk.CTkFont(size=18, weight="bold"),
            text_color=get_color("primary")
        )
        title.pack(pady=(20, 10))
        ...
        # Content
        content = """Enterprise Edition v2.1

A comprehensive solution for managing access 
reviews in Cognos environments with support for 
automated file generation, email delivery, and 
response tracking.
